...
template<typename T>
ParamEditor<T>::ParamEditor() {
    _soloParamRepository = *(uint64_t*)Global::SOLO_PARAM_REPOSITORY.ptr();
    ...
    if (strcmp(paramType.c_str(), T::param_type) != 0) {
        printf("Param %s def strings name did not match. game: %s header: %s\n", std::string(T::param_name).c_str(), paramType, std::string(T::param_type).c_str());
        return;
    }
